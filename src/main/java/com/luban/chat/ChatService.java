...
//            System.out.println("2秒了");
      ...
      while (iterator.hasNext()) {
        ...
        if (selectionKey.isAcceptable()) {     //连接事件
          ...
          out.println(accept.getRemoteAddress().toString() + "上线了");
        }
  ...
  private void readClientData(SelectionKey selectionKey) throws IOException {
    ...
    out.println("key.interestOps() = " + selectionKey.interestOps() +
        "socketChannel.getRemoteAddress() = " + socketChannel.getRemoteAddress() +
        "socketChannel.isConnected()  = " + socketChannel.isConnected());
    ByteBuffer byteBuffer = ByteBuffer.allocate(1024);
    int read = socketChannel.read(byteBuffer);
    out.println("read = " + read);
    if (read <= 0) {
      String address = socketChannel.getRemoteAddress().toString();
      out.println(address + " will close");
      socketChannel.close();
      selectionKey.cancel();
      out.println(address + " closed");
      return;
    }
    byteBuffer.flip();
    if (read > 0) {
  ...
  private void writeClientData(SocketChannel socketChannel, String s) throws IOException {
    ...
    for (SelectionKey key : keys) {
      out.println("keys->key.interestOps() = " + key.interestOps());
      if (key.isValid()) {
        SelectableChannel channel = key.channel();
        if (channel instanceof SocketChannel) {
          SocketChannel socketChannel1 = (SocketChannel) channel;
          if (channel != socketChannel) {
            s = socketChannel.getRemoteAddress().toString() + ":" + s;
            ByteBuffer wrap = ByteBuffer.wrap(s.getBytes());
            out.println("Write to " + socketChannel1.getRemoteAddress() + ": " + s);
